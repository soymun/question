...
public interface TaskHistoryResultRepository extends JpaRepository<TaskHistoryResult, Long> {
    ...
    @Query(value = "from TaskHistoryResult tr where tr.task.id=:taskId ORDER BY tr.timeResult desc")
    List<TaskHistoryResult> getAllByTaskId(@Param("taskId") Long id);

    @Query(value = "from TaskHistoryResult tr where tr.task.id=:taskId and tr.user.id=:userId ORDER BY tr.timeResult desc ")
    List<TaskHistoryResult> getAllByTaskIdAndUserId(@Param("taskId") Long id, @Param("userId") Long userId);

    @Query(value = "from TaskHistoryResult tr where tr.user.id=:userId ORDER BY tr.timeResult desc")
